...
When wall-jumping, the player oftenhas to rapidly switch from pushing
...
This option enables the Float mechanicand determines what key the
...
When people jump in real life, they can’tchange direction mid-air.
...
By default, GBStudiokeeps your avatar oriented in the
...
By default, the GBS Platform Enginedoes something similar to
...
### Run Style  
By default, the GBS run command adds asmall amount of acceleration to
...
By default, GBStudio allowsyour character to turn
...
This option allows youto add more height to a jump
...
This option enables or disables dashingin your game, and determines
...
Allows you to select if the player can dash only when touching the ground,
...
### Dash Momentum:
Dashing, unlike regular movement, triesto travel a fixed distance
...
### Dash Through : 
This setting allows you to change howthe dash interacts with objects
...
The total distance covered by the dash. Divide this by Dash Time to get
...
The time it takes for the dashto cross its total distance. This is
...
The time (in frames) before theplayer can dash again after
